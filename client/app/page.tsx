...
const UserAvatar = memo(({ src, alt, className, fallbackClass }: any) => {
  return src ? (
    <img src={src} alt={alt || "User"} className={`${className} bg-black/50`} loading="eager" />
  ) : (
    <div className={`${className} ${fallbackClass || "bg-white/5 backdrop-blur-md"} flex items-center justify-center`}>
...
const GifPicker = ({ onSelect, onClose }: any) => {
  ...
  return (
    <div className="absolute bottom-24 left-4 w-[360px] h-[480px] bg-[#121212]/90 backdrop-blur-2xl border border-white/10 rounded-[32px] shadow-2xl flex flex-col z-50 animate-scale-up overflow-hidden ring-1 ring-white/5">
      <div className="p-4 border-b border-white/5 flex gap-3 items-center bg-white/5">
        <div className="relative flex-1">
            <input 
            className="w-full bg-black/40 text-white px-4 py-2.5 rounded-2xl text-sm focus:outline-none focus:ring-1 focus:ring-blue-500/50 border border-white/5 placeholder-white/30 font-medium transition-all"
      ...
      <div className="flex-1 overflow-y-auto p-3 custom-scrollbar">
        {loading ? <div className="flex h-full items-center justify-center text-xs font-bold text-white/30 uppercase animate-pulse">Loading...</div> : (
          <div className="columns-2 gap-2 space-y-2">
            {gifs.map((g) => (
              <div key={g.id} className="relative group overflow-hidden rounded-xl cursor-pointer hover:ring-2 hover:ring-blue-500/50 transition-all" onClick={() => onSelect(getGifUrl(g))}>
...
export default function DaChat() {
  ...
  // 🔥 LOGIN SCREEN (Glass)
  if (!user) return ( 
    <div className="flex h-screen items-center justify-center bg-[#050505] bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-blue-900/20 via-[#050505] to-[#050505] text-white font-sans"> 
      <div className="z-10 bg-black/40 backdrop-blur-2xl border border-white/5 p-10 rounded-[32px] w-[380px] text-center shadow-2xl ring-1 ring-white/10 flex flex-col gap-6"> 
        <div className="mb-2">
            ...
            <h1 className="text-3xl font-bold tracking-tight text-white">DaChat</h1> 
            <p className="text-white/40 text-sm mt-2 font-medium">Your world, organized.</p>
        </div>
        {error && <p className="text-red-300 text-xs bg-red-500/10 py-3 rounded-xl border border-red-500/10 font-medium">{error}</p>} 
        <div className="space-y-3">
            <input className="w-full bg-black/50 border border-white/5 text-white px-5 py-3.5 rounded-2xl focus:outline-none focus:ring-2 focus:ring-blue-500/50 transition-all placeholder-white/20 text-sm font-medium backdrop-blur-sm" placeholder="Username" onChange={e => setAuthForm({ ...authForm, username: e.target.value })} /> 
            <input className="w-full bg-black/50 border border-white/5 text-white px-5 py-3.5 rounded-2xl focus:outline-none focus:ring-2 focus:ring-blue-500/50 transition-all placeholder-white/20 text-sm font-medium backdrop-blur-sm" type="password" placeholder="Password" onChange={e => setAuthForm({ ...authForm, password: e.target.value })} /> 
        </div>
        <button onClick={handleAuth} className="w-full bg-white text-black hover:bg-zinc-200 py-3.5 rounded-2xl font-bold transition-all shadow-xl shadow-white/5 mt-2 text-sm active:scale-95">
  ...
  return (
    <div className="flex h-screen bg-[#050505] bg-[radial-gradient(circle_at_top_right,_var(--tw-gradient-stops))] from-indigo-900/20 via-[#050505] to-[#050505] text-zinc-200 font-sans overflow-hidden p-2 gap-2">
      
      {/* 1. DOCK (Floating Glass) */}
      <div className="w-[80px] flex flex-col items-center py-6 gap-3 z-30 rounded-[32px] bg-black/40 backdrop-blur-xl border border-white/5 shadow-2xl">
        <div onClick={goToHome} className={`w-[50px] h-[50px] rounded-[20px] flex items-center justify-center cursor-pointer transition-all duration-300 mb-4 shadow-lg border ${view === 'dms' ? "bg-blue-600 border-blue-500 text-white shadow-blue-900/20" : "bg-white/5 border-transparent text-white/40 hover:bg-white/10 hover:text-white"}`}> 
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-6 h-6"><path strokeLinecap="round" strokeLinejoin="round" d="M2.25 12l8.954-8.955c.44-.439 1.152-.439 1.591 0L21.75 12M4.5 9.75v10.125c0 .621.504 1.125 1.125 1.125H9.75v-4.875c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125V21h4.125c.621 0 1.125-.504 1.125-1.125V9.75M8.25 21h8.25" /></svg> 
        </div>
        
        <div className="w-10 h-[2px] bg-white/5 rounded-full mb-2" />
        
        <div className="flex-1 w-full flex flex-col items-center gap-3 overflow-y-auto no-scrollbar px-2 pb-2">
        {servers.map(s => ( 
            <div key={s.id} onClick={() => selectServer(s)} className={`group relative w-[50px] h-[50px] rounded-[20px] flex items-center justify-center cursor-pointer transition-all duration-300 shadow-md border ${active.server?.id === s.id ? "bg-blue-600 border-blue-500 text-white" : "bg-white/5 border-transparent text-white/50 hover:bg-white/10 hover:text-white"}`}> 
                <span className="font-bold text-sm tracking-tight">{s.name.substring(0, 2).toUpperCase()}</span> 
                {active.server?.id === s.id && <div className="absolute -left-[18px] w-1.5 h-8 bg-white rounded-r-full shadow-[0_0_10px_rgba(255,255,255,0.5)]" />} 
            </div> 
        ))}
        <div onClick={createServer} className="w-[50px] h-[50px] rounded-[20px] border border-dashed border-white/20 bg-transparent flex items-center justify-center cursor-pointer text-white/30 hover:border-green-500 hover:text-green-500 transition-all mt-2"> <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6"><path strokeLinecap="round" strokeLinejoin="round" d="M12 4.5v15m7.5-7.5h-15" /></svg> </div>
        </div>
      </div>

      {/* 2. SIDEBAR (Floating Glass) */}
      <div className="w-[260px] rounded-[32px] bg-black/40 backdrop-blur-xl border border-white/5 flex flex-col shadow-2xl overflow-hidden">
        <div className="h-20 flex items-center px-6 font-bold text-white text-[16px] tracking-tight border-b border-white/5 bg-white/[0.02]"> {active.server ? active.server.name : "Messages"} </div>
        <div className="flex-1 p-4 overflow-y-auto">
          {view === "servers" && active.server ? (
            <>
              <div className="flex justify-between items-center px-2 pb-3 pt-2 text-[10px] font-bold text-white/30 uppercase tracking-widest"> <span>Channels</span> <button onClick={createChannel} className="hover:text-white transition-colors text-lg leading-none opacity-50 hover:opacity-100">+</button> </div>
              <div className="space-y-1">
                {channels.map(ch => (
                  <div key={ch.id} className={`group flex flex-col px-4 py-2.5 rounded-2xl cursor-pointer transition-all duration-200 border ${active.channel?.id === ch.id ? "bg-white/10 border-white/5 text-white" : "border-transparent hover:bg-white/5 text-white/50 hover:text-white"}`}>
            ...
            <>
              <button onClick={addFriend} className="w-full bg-white/5 hover:bg-white/10 text-white/70 border border-white/5 text-xs font-semibold py-3 rounded-xl mb-6 transition-all tracking-wide shadow-sm">Find Friends</button>
              <div className="text-[10px] font-bold text-white/30 uppercase tracking-widest px-2 mb-3">Direct Messages</div>
              {friends.map(f => (
                <div key={f.id} onClick={() => selectFriend(f)} className={`px-3 py-3 rounded-2xl cursor-pointer flex gap-3 items-center transition-all duration-200 border ${active.friend?.id === f.id ? "bg-white/10 border-white/5" : "border-transparent hover:bg-white/5"}`}>
                  <UserAvatar src={f.avatar_url} className="w-10 h-10 rounded-full object-cover bg-black/50" />
                  <div className="flex flex-col"> <span className="text-[13px] font-medium text-zinc-200">{f.username}</span> <span className="text-[10px] text-white/40 font-medium">Offline</span> </div>
                </div>
              ))}
            </>
          )}
        ...
        <div className="p-4 bg-black/20 border-t border-white/5">
          <div className="flex items-center justify-between p-2 rounded-2xl hover:bg-white/5 transition-colors group cursor-pointer" onClick={openSettings}>
            <div className="flex items-center gap-3">
              <UserAvatar src={user.avatar_url} className="w-10 h-10 rounded-full object-cover bg-black/50" />
      ...
      {/* 3. MAIN CHAT / VIDEO (Floating Glass) */}
      <div className="flex-1 rounded-[32px] bg-black/40 backdrop-blur-xl border border-white/5 flex flex-col min-w-0 relative shadow-2xl overflow-hidden">
        {(active.channel || active.friend) ? (
          <>
            {/* TOP BAR */}
            <div className="h-20 border-b border-white/5 flex items-center justify-between px-8 bg-white/[0.02] z-10 sticky top-0 backdrop-blur-sm">
              <div className="flex items-center gap-4">
                <span className="text-white/30 text-2xl font-light">{active.channel ? (active.channel.type==='voice'? <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-6 h-6"><path strokeLinecap="round" strokeLinejoin="round" d="M19.114 5.636a9 9 0 010 12.728M16.463 8.288a5.25 5.25 0 010 7.424M6.75 8.25l4.72-4.72a.75.75 0 011.28.53v15.88a.75.75 0 01-1.28.53l-4.72-4.72H4.51c-.88 0-1.704-.507-1.938-1.354A9.01 9.01 0 012.25 12c0-.83.112-1.633.322-2.396C2.806 8.756 3.63 8.25 4.51 8.25H6.75z" /></svg> : '#') : "@"}</span>
                <span className="font-bold text-white text-lg tracking-wide">{active.channel ? active.channel.name : active.friend.username}</span>
              </div>
              {!active.channel && active.friend && (
                <div onClick={startDMCall} className="bg-green-600/80 hover:bg-green-500 text-white p-3 rounded-full cursor-pointer transition-all shadow-lg shadow-green-900/20 backdrop-blur-md"> <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-5 h-5"><path strokeLinecap="round" strokeLinejoin="round" d="M2.25 6.75c0 8.284 6.716 15 15 15h2.25a2.25 2.25 0 002.25-2.25v-1.372c0-.516-.351-.966-.852-1.091l-4.423-1.106c-.44-.11-.902.055-1.173.417l-.97 1.293c-.282.376-.769.542-1.21.38a12.035 12.035 0 01-7.143-7.143c-.162-.441.004-.928.38-1.21l1.293-.97c.363-.271.527-.734.417-1.173L6.963 3.102a1.125 1.125 0 00-1.091-.852H4.5A2.25 2.25 0 002.25 4.5v2.25z" /></svg> </div>
              )}
            </div>

            {/* MAXIMIZED VIDEO */}
            {maximizedContent && (
                <div className="absolute inset-0 bg-black/95 z-50 flex items-center justify-center animate-scale-up backdrop-blur-2xl">
            ...
            {inCall ? (
              <div className="flex-1 flex flex-col items-center justify-center relative p-8">
                <div className="grid grid-cols-2 gap-6 w-full h-full max-w-6xl">
                  {/* MY VIDEO */}
                  <div className="bg-black/40 rounded-[40px] flex flex-col items-center justify-center relative overflow-hidden border border-white/5 shadow-2xl group backdrop-blur-md"> 
                    <div className="w-full h-full absolute inset-0 flex items-center justify-center">
                        {isScreenSharing ? (
                            ...
                            <div className="flex flex-col items-center">
                                ...
                                <div className="flex items-center gap-2 mt-3 px-4 py-1.5 bg-green-500/10 rounded-full border border-green-500/20 backdrop-blur-md">
                    ...
                    {isScreenSharing && (
                        <>
                            <div className="absolute top-6 right-6 bg-red-500/80 hover:bg-red-500 text-white px-5 py-2.5 rounded-xl text-xs font-bold cursor-pointer shadow-lg z-10 transition-all tracking-wide border border-white/10 backdrop-blur-md" onClick={() => stopScreenShare()}>STOP SHARING</div>
                            <div onClick={() => setMaximizedContent({ stream: myVideoRef.current!.srcObject as MediaStream, type: 'local' })} className="absolute top-6 left-6 bg-black/60 hover:bg-black/80 text-white p-3 rounded-xl opacity-0 group-hover:opacity-100 transition-all cursor-pointer border border-white/10 backdrop-blur-md z-10"> <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5"><path strokeLinecap="round" strokeLinejoin="round" d="M3.75 3.75v4.5m0-4.5h4.5m-4.5 0L9 9M3.75 20.25v-4.5m0 4.5h4.5m-4.5 0L9 15M20.25 3.75h-4.5m4.5 0v4.5m0-4.5L15 9m5.25 11.25h-4.5m4.5 0v-4.5m0 4.5L15 15" /></svg> </div>
                        </>
                    )}
                  </div>
                  
                  {peers.map((p, i) => ( 
                    <div key={i} className="bg-black/40 rounded-[40px] flex flex-col items-center justify-center border border-white/5 relative overflow-hidden group shadow-2xl backdrop-blur-md"> 
                ...
                {/* Floating Controls */}
                <div className="absolute bottom-10 flex gap-4 p-4 bg-black/60 border border-white/10 rounded-[32px] shadow-2xl backdrop-blur-xl z-50">
                    ...
                    <button onClick={leaveCall} className="w-14 h-14 rounded-2xl bg-red-500 hover:bg-red-400 text-white flex items-center justify-center shadow-lg shadow-red-900/30 transition-all">
              ...
              <>
                  ...
                  {chatHistory.map((msg, i) => (
                    <div key={i} className="flex gap-4 group animate-fade-in-up">
                      ...
                      <div className="flex-1">
                        <div className="flex items-baseline gap-3 mb-1"> <span className={`font-bold text-[15px] cursor-pointer hover:underline ${msg.sender_id === user.id ? "text-blue-400" : "text-zinc-300"}`} onClick={() => viewUserProfile(msg.sender_id)}>{msg.sender_name}</span> <span className="text-[11px] text-white/30 font-medium">Today at 12:00 PM</span> </div>
                        {msg.content && (
                            ...
                            <p className="text-zinc-300 text-[15px] leading-relaxed font-normal bg-white/5 inline-block px-4 py-2 rounded-2xl rounded-tl-none">{msg.content}</p>
                        )}
                ...
                <div className="p-6 relative"> 
                  ...
                  <div className="bg-white/5 border border-white/10 rounded-[24px] px-3 py-2 flex items-center gap-3 shadow-xl backdrop-blur-xl relative z-20"> 
                    <div className="w-10 h-10 rounded-xl bg-white/5 hover:bg-white/10 flex items-center justify-center cursor-pointer text-white/50 hover:text-white transition-all ml-1" onClick={() => fileInputRef.current?.click()}> 
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-5 h-5"><path strokeLinecap="round" strokeLinejoin="round" d="M12 4.5v15m7.5-7.5h-15" /></svg> 
                    </div> 
                    <div className="w-10 h-10 rounded-xl bg-white/5 hover:bg-white/10 flex items-center justify-center cursor-pointer text-white/50 hover:text-white transition-all text-[10px] font-black tracking-wider" onClick={() => setShowGifPicker(!showGifPicker)}>GIF</div> 
                    <input type="file" ref={fileInputRef} className="hidden" onChange={handleFileUpload} accept="image/*" /> 
                    <input className="bg-transparent flex-1 outline-none text-zinc-200 placeholder-white/30 font-medium px-2 text-[15px]" placeholder={`Message ${active.channel ? "#"+active.channel.name : "@"+active.friend.username}`} value={message} onChange={e => setMessage(e.target.value)} onKeyDown={e => e.key === 'Enter' && sendMessage(message)} disabled={isUploading} /> 
                  </div> 
                </div>
              </>
            )}
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-white/20 gap-6"> 
            <div className="w-24 h-24 rounded-[32px] bg-white/5 border border-white/5 flex items-center justify-center shadow-inner backdrop-blur-sm"> 
      ...
      {/* 4. MEMBER LIST (Floating Glass) */}
      {view === "servers" && active.server && (
        <div className="w-[260px] rounded-[32px] bg-black/40 backdrop-blur-xl border border-white/5 flex flex-col shadow-2xl flex-shrink-0 overflow-hidden">
          ...
          <div className="flex-1 p-5 overflow-y-auto custom-scrollbar">
             ...
             {serverMembers.map(member => (
               <div key={member.id} className="flex items-center gap-3 mb-2 p-2.5 rounded-2xl hover:bg-white/5 group transition-colors cursor-pointer" onClick={() => viewUserProfile(member.id)}>
                 <UserAvatar src={member.avatar_url} className="w-9 h-9 rounded-full object-cover shadow-sm bg-black/50" />
                 <div className="flex-1 min-w-0"> 
                    ...
                    <div className="text-[10px] text-white/40 font-medium">#{member.discriminator}</div> 
                 </div>
               </div>
             ))}
          </div>
      ...
      {showSettings && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-md animate-fade-in">
          <div className="bg-[#121212]/90 w-[420px] rounded-[40px] border border-white/10 shadow-2xl overflow-hidden animate-scale-up ring-1 ring-white/10 backdrop-blur-xl">
            ...
            <div className="p-10 flex flex-col gap-8">
              ...
              <div className="space-y-5">
                <div className="flex flex-col gap-2"> <label className="text-[11px] font-bold text-white/30 uppercase tracking-widest ml-1">Username</label> <input value={newUsername} onChange={(e) => setNewUsername(e.target.value)} className="bg-black/40 border border-white/10 text-white p-4 rounded-2xl focus:outline-none focus:ring-1 focus:ring-blue-500/50 transition-all font-medium backdrop-blur-sm" /> </div>
                <div className="flex flex-col gap-2"> <label className="text-[11px] font-bold text-white/30 uppercase tracking-widest ml-1">About Me</label> <textarea value={newBio} onChange={(e) => setNewBio(e.target.value)} rows={3} className="bg-black/40 border border-white/10 text-white p-4 rounded-2xl focus:outline-none focus:ring-1 focus:ring-blue-500/50 transition-all resize-none font-medium backdrop-blur-sm" placeholder="Write something..." /> </div>
              </div>
            </div>
      ...
      {viewingProfile && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-md animate-fade-in" onClick={() => setViewingProfile(null)}>
          <div className="bg-[#121212]/90 w-[380px] rounded-[40px] border border-white/10 shadow-2xl overflow-hidden animate-scale-up ring-1 ring-white/10 backdrop-blur-xl" onClick={e => e.stopPropagation()}>
            <div className="h-32 bg-gradient-to-tr from-blue-900 to-indigo-900 relative"> <div className="absolute -bottom-12 left-1/2 -translate-x-1/2 p-2 bg-[#121212] rounded-full"> <UserAvatar src={viewingProfile.avatar_url} className="w-28 h-28 rounded-full object-cover border-4 border-[#121212]" /> </div> </div>
            ...
            <>
                <div className="absolute top-4 right-4 z-30 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                    <button 
                        ...
                        className="bg-black/60 hover:bg-blue-600 text-white p-2.5 rounded-xl backdrop-blur-md border border-white/20 shadow-xl transition-all active:scale-95 flex items-center gap-2"
                    >
