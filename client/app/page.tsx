...
const RoomPlayer = memo(({ track, isDj, socket, roomId, showQueue, setShowQueue }: any) => {
    ...
    const handleVolume = (e: any) => {
        ...
        setVolume(v);
        setIsMuted(v === 0);
        // If using an audio element, set it here. 
        // For sync listening, this usually just affects local preview or passed to player.
    };

    return (
        <div className="flex flex-col bg-[#1e1f22] border-t border-white/10 shrink-0 relative">
            ...
            <div className="grid grid-cols-3 items-center px-4 py-3 h-20 gap-4">
                
                {/* 1. LEFT: SONG INFO (Image + Text) */}
                ...
                <div className="flex flex-col items-center justify-center gap-1">
                    <div className="flex items-center gap-6">
                        {/* Skip Back (Visual only or restart) */}
                        ...
                        {/* Skip Forward */}
                        <button onClick={skipTrack} className={`text-gray-400 hover:text-white transition ${!isDj && "opacity-50 cursor-not-allowed"}`}>
            ...
            {/* QUEUE POPUP (Positioned absolutely above the player) */}
            {showQueue && (
