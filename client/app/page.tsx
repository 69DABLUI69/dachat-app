  ...
  if (!user) return (
    <div className="flex h-screen items-center justify-center bg-black relative overflow-hidden p-0 md:p-4">
      ...
      <GlassPanel className="p-10 w-full h-full md:h-auto md:max-w-100 rounded-none md:rounded-[40px] text-center relative z-10 flex flex-col justify-center gap-6 ring-1 ring-white/10 animate-in fade-in zoom-in-95 duration-500">
                ...
                <>
                    ...
                    {!isRegistering && (
                        <div className="flex items-center gap-2 px-2">
                            ...
                            <label htmlFor="remember" className="text-xs text-white/50 cursor-pointer select-none hover:text-white transition-colors">Remember me</label>
                        </div>
                    )}
                </>
            ) : (
        ...
        {!is2FALogin && <p className="text-xs text-white/40 cursor-pointer hover:text-white transition-colors" onClick={() => setIsRegistering(!isRegistering)}>{isRegistering ? "Back to Login" : "Create an Account"}</p>}
      </GlassPanel>
    </div>
  );
                        ...
                        {chatHistory.map((msg, i) => ( 
                            <div key={msg.id || i} className={`flex gap-3 animate-in slide-in-from-bottom-2 fade-in duration-300 ${msg.sender_id === user.id ? "flex-row-reverse" : ""}`} onContextMenu={(e) => handleContextMenu(e, 'message', msg)}> 
                                ...
                                <div className={`max-w-[85%] md:max-w-[70%] ${msg.sender_id===user.id?"items-end":"items-start"} flex flex-col`}> 
                                    ...
                                    <div className={`group px-4 py-2 rounded-2xl text-sm shadow-md cursor-pointer transition-all hover:scale-[1.01] ${msg.sender_id===user.id?"bg-blue-600":"bg-white/10"}`}> {msg.content?.startsWith("http") ? <img src={msg.content} className="max-w-50 md:max-w-62.5 rounded-lg transition-transform hover:scale-105" /> : msg.content} </div> 
...
// ... [RoomPlayer & MediaPlayer Components REMAIN UNCHANGED] ...
    ...
    return (
        <div className="bg-linear-to-b from-indigo-900/50 to-black/50 border-t border-white/10 p-4 flex flex-col gap-3 backdrop-blur-md">
            {track ? (
                <div className="flex gap-3 items-center animate-in slide-in-from-bottom-2">
                    ...
                    <div className="flex-1 min-w-0 flex flex-col justify-center">
                        ...
                        <div className="text-[10px] text-indigo-300 flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse"/> Playing for everyone</div>
                    </div>
            ...
            <div className="relative group">
                <input className="w-full bg-black/40 border border-white/5 rounded-lg px-3 py-2 text-xs text-white placeholder-white/20 focus:outline-none focus:border-indigo-500/50 transition-all" placeholder="Search YouTube to Play..." value={search} onChange={(e) => setSearch(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter' && search.trim()) { onSearch(search); setSearch(""); }}}/>
