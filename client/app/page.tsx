    ...
    <GlassPanel className="absolute bottom-24 left-4 w-[360px] h-[480px] rounded-[32px] flex flex-col z-50 overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-300">
      ...
      <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
        <div className="columns-2 gap-3 space-y-3">
          {gifs.map((g) => ( <div key={g.id} className="relative group overflow-hidden rounded-2xl cursor-pointer hover:ring-2 ring-blue-500/50 transition-all" onClick={() => onSelect(g?.media_formats?.gif?.url)}> <img src={g?.media_formats?.tinygif?.url} className="w-full h-auto object-cover rounded-xl" /> </div> ))}
        </div>
      </div>
    </GlassPanel>
  );
...
export default function DaChat() {
  ...
  const selectServer = async (server: any) => {
    setView("servers");
    setActive({ ...active, server, friend: null, pendingRequest: null });
  ...
  const saveProfile = async () => {
      ...
      if (newAvatarFile) {
          const formData = new FormData(); formData.append("file", newAvatarFile);
          try { const res = await fetch(`${BACKEND_URL}/upload`, { method: "POST", body: formData }); const data = await res.json(); if (data.success) finalAvatarUrl = data.fileUrl; else { alert("Upload failed"); return; } } catch { alert("Error uploading"); return; }
      }
      try {
        ...
        const data = await res.json();
        if (data.success) { setUser(data.user); setShowSettings(false); setNewAvatarFile(null); alert("Updated!"); } else { alert(`Failed: ${data.message}`); }
  ...
  const joinVoiceRoom = useCallback((roomId: string) => {
    ...
    navigator.mediaDevices.getUserMedia({ video: false, audio: true }).then(stream => { setInCall(true); setMyStream(stream); socket.emit("join_voice", { roomId, userData: user });
    ...
    }).catch(err => { console.error(err); alert("Mic denied"); });
  }, [user]);

  const createPeer = (u:string, c:string, s:MediaStream, d:any) => { const p = new Peer({ initiator: true, trickle: false, stream: s }); p.on("signal", (sig:any) => socket.emit("sending_signal", { userToSignal: u, callerID: c, signal: sig, userData: user })); return p; };
  const addPeer = (sig:any, c:string, s:MediaStream) => { const p = new Peer({ initiator: false, trickle: false, stream: s }); p.on("signal", (sig:any) => socket.emit("returning_signal", { signal: sig, callerID: c })); p.signal(sig); return p; };
  ...
  if (!user) return (
    <div className="flex h-screen items-center justify-center bg-black relative overflow-hidden">
      <div className="absolute inset-0 bg-gradient-to-br from-indigo-900 via-purple-900 to-black opacity-40"></div>
      <GlassPanel className="p-10 rounded-[40px] w-[400px] text-center relative z-10 flex flex-col gap-6 ring-1 ring-white/10">
        <div> <h1 className="text-3xl font-bold text-white">DaChat</h1> </div>
        {error && <div className="text-red-400 text-xs">{error}</div>}
        <input className="w-full bg-black/30 border border-white/5 text-white px-5 py-4 rounded-2xl" placeholder="Username" onChange={e => setAuthForm({ ...authForm, username: e.target.value })} />
        <input className="w-full bg-black/30 border border-white/5 text-white px-5 py-4 rounded-2xl" type="password" placeholder="Password" onChange={e => setAuthForm({ ...authForm, password: e.target.value })} />
        <button onClick={handleAuth} className="w-full bg-white text-black py-4 rounded-2xl font-bold">{isRegistering ? "Create" : "Login"}</button>
        <p className="text-xs text-white/40 cursor-pointer" onClick={() => setIsRegistering(!isRegistering)}>{isRegistering ? "Login" : "Register"}</p>
      </GlassPanel>
    </div>
  );
  ...
  return (
    <div className="flex h-screen w-screen bg-[#050505] text-white font-sans overflow-hidden relative selection:bg-blue-500/30">
      ...
      <div className="z-30 w-[90px] h-full flex flex-col items-center py-8 gap-4 fixed left-0 top-0">
        <GlassPanel onClick={() => { setView("dms"); setActive({server:null,friend:null,channel:null}); }} className={`w-[56px] h-[56px] rounded-[24px] flex items-center justify-center cursor-pointer transition-all duration-300 ${view === 'dms' ? "bg-blue-600/80 border-blue-400/50 shadow-[0_0_30px_rgba(37,99,235,0.4)]" : "hover:bg-white/10"}`}> <span className="font-bold">DM</span> </GlassPanel>
        ...
        <div className="flex-1 w-full flex flex-col items-center gap-4 overflow-y-auto no-scrollbar pb-4 px-2">
            {servers.map(s => ( <div key={s.id} onClick={() => selectServer(s)} className="group relative w-full flex justify-center cursor-pointer"> {active.server?.id === s.id && <div className="absolute -left-1 top-1/2 -translate-y-1/2 w-1 h-8 bg-white rounded-r-full shadow-[0_0_15px_white]" />} <GlassPanel className={`w-[56px] h-[56px] rounded-[24px] flex items-center justify-center transition-all duration-300 ${active.server?.id === s.id ? "bg-gradient-to-br from-blue-600 to-purple-600 border-white/20 rounded-[20px]" : "hover:bg-white/10 hover:rounded-[20px]"}`}> <span className="font-bold text-sm">{s.name.substring(0, 2).toUpperCase()}</span> </GlassPanel> </div> ))}
            ...
            <div className="flex-1 p-4 overflow-y-auto custom-scrollbar space-y-1">
                {view === "servers" && active.server ? (
                    ...
                    <>
                        ...
                        {requests.map(req => (
                            <div key={req.id} onClick={() => selectRequest(req)} className={`p-3 rounded-[24px] flex items-center gap-4 cursor-pointer transition-all ${active.pendingRequest?.id === req.id ? "bg-blue-600/20 border border-blue-500/50" : "bg-white/5 hover:bg-white/10"}`}>
                                <UserAvatar src={req.avatar_url} className="w-10 h-10 rounded-[16px]" />
                                <div><div className="text-sm font-bold text-white/90">{req.username}</div><div className="text-[10px] text-yellow-400 font-bold">Pending</div></div>
                            </div>
                        ))}

                        <div className="flex justify-between items-center px-4 py-4 text-[11px] font-bold text-white/40 uppercase tracking-widest mt-4"> <span>Friends</span> </div>
         ...
         <GlassPanel className="w-full h-full rounded-[40px] flex flex-col relative overflow-hidden ring-1 ring-white/5">
            ...
            {active.pendingRequest ? (
                <div className="flex-1 flex flex-col items-center justify-center p-10">
                    ...
                    <h2 className="text-3xl font-bold mb-2">{active.pendingRequest.username}</h2>
                    <p className="text-white/50 mb-8">wants to add you as a friend.</p>
                    <div className="flex gap-4">
                        <button onClick={handleAcceptRequest} className="px-8 py-3 bg-green-600 hover:bg-green-500 rounded-xl font-bold text-white transition-all shadow-[0_0_20px_rgba(34,197,94,0.4)]">Accept</button>
                        <button onClick={handleDeclineRequest} className="px-8 py-3 bg-red-600/20 hover:bg-red-600/40 text-red-200 border border-red-500/30 rounded-xl font-bold transition-all">Decline</button>
                    </div>
                </div>
                ...
                <div className="flex-1 flex flex-col items-center justify-center text-white/20"> <div className="w-32 h-32 rounded-[40px] border-4 border-white/5 flex items-center justify-center mb-6"> <svg xmlns="http://www.w3.org/2000/svg" className="h-12 w-12" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" /></svg> </div> <p className="font-light tracking-widest uppercase">Select a channel or request</p> </div>
            )}
         </GlassPanel>
      </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
              <GlassPanel className="w-[450px] p-8 flex flex-col relative">
                  ...
                  <div className="flex flex-col items-center gap-4 mb-6">
                      ...
                      <button onClick={() => setShowProfileGifPicker(!showProfileGifPicker)} className="text-[10px] bg-white/10 hover:bg-white/20 px-3 py-1 rounded-full text-white/70 font-bold border border-white/5 transition-all">CHOOSE GIF</button>
                  ...
                  {showProfileGifPicker && (
                    <div className="absolute inset-0 z-50 bg-[#0e0e0f] rounded-[32px] overflow-hidden flex flex-col">
                        <div className="p-4 border-b border-white/10 flex justify-between items-center"><span className="font-bold text-sm">Select GIF</span><button onClick={() => setShowProfileGifPicker(false)}>✕</button></div>
                        <div className="flex-1 relative"><GifPicker onSelect={(url: string) => { setEditForm({ ...editForm, avatarUrl: url }); setNewAvatarFile(null); setShowProfileGifPicker(false); }} onClose={() => setShowProfileGifPicker(false)} /></div>
                    </div>
                  )}
                  <div className="space-y-4">
                      <div><label className="text-xs font-bold text-white/40 uppercase ml-1">Username</label><input className="w-full bg-black/30 border border-white/10 text-white px-4 py-3 rounded-xl mt-1 focus:outline-none focus:border-white/30" value={editForm.username} onChange={e => setEditForm({...editForm, username: e.target.value})} /></div>
                      <div><label className="text-xs font-bold text-white/40 uppercase ml-1">Bio</label><textarea className="w-full bg-black/30 border border-white/10 text-white px-4 py-3 rounded-xl mt-1 focus:outline-none focus:border-white/30 resize-none h-24" value={editForm.bio} onChange={e => setEditForm({...editForm, bio: e.target.value})} placeholder="Tell us about yourself..." /></div>
                  </div>
