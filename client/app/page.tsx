...
export default function DaChat() {
  ...
  return (
    <div className="flex h-screen w-screen bg-[#050505] text-white font-sans overflow-hidden relative selection:bg-blue-500/30">
      ...
      <div className="flex-1 h-screen relative z-10 p-4 pl-0 min-w-0">
         <GlassPanel className="w-full h-full rounded-[40px] flex flex-col relative overflow-hidden ring-1 ring-white/5">
            ...
            {active.pendingRequest ? (
                ...
                <>
                     {/* CHAT HEADER */}
                    <div className="h-20 border-b border-white/5 flex items-center justify-between px-8 bg-white/[0.02]"> <div className="flex items-center gap-4"> <span className="text-2xl text-white/30 font-light">@</span> <span className="text-lg font-bold tracking-wide cursor-pointer hover:underline" onClick={() => active.friend ? viewUserProfile(active.friend.id) : null}>{active.channel ? active.channel.name : active.friend?.username}</span> </div> 
                    {/* ✅ UPDATED: Call Button triggers startDMCall */}
                    {!active.channel && active.friend && ( <button onClick={startDMCall} className="w-10 h-10 rounded-full bg-green-500/20 text-green-400 hover:bg-green-500 hover:text-white flex items-center justify-center transition-all"> <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" /></svg> </button> )} </div>
                    {/* CHAT CONTENT */}
                    {inCall ? (
      ...
      {/* ✅ NEW: INCOMING CALL MODAL */}
      {incomingCall && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 backdrop-blur-md animate-in zoom-in-95 duration-200">
              <div className="flex flex-col items-center">
                  <UserAvatar src={incomingCall.avatarUrl} className="w-32 h-32 rounded-full border-4 border-white/20 mb-6 shadow-[0_0_50px_rgba(255,255,255,0.2)] animate-pulse" />
                  <h2 className="text-3xl font-bold text-white mb-2">{incomingCall.senderName}</h2>
                  <p className="text-white/50 mb-10 tracking-widest uppercase text-xs">Incoming Video Call...</p>
                  <div className="flex gap-8">
                      <button onClick={() => setIncomingCall(null)} className="w-16 h-16 rounded-full bg-red-500 hover:bg-red-600 flex items-center justify-center text-white shadow-lg transition-transform hover:scale-110">
                          <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                      </button>
                      <button onClick={answerCall} className="w-16 h-16 rounded-full bg-green-500 hover:bg-green-600 flex items-center justify-center text-white shadow-lg transition-transform hover:scale-110">
