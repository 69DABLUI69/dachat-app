...
export default function DaChat() {
  ...
  useEffect(() => { 
      ...
      socket.on("user_updated", ({ userId }) => { if (viewingProfile?.id === userId) viewUserProfile(userId); if (active.server) fetchServers(user.id); fetchFriends(user.id); });
      ...
      return () => { 
          socket.off("receive_message"); socket.off("load_messages"); socket.off("voice_state_update"); socket.off("user_updated"); socket.off("new_friend_request"); socket.off("incoming_call"); socket.off("server_updated"); socket.off("new_server_invite"); 
          socket.off("call_ended"); // Clean listener
      }; 
  }, [user, viewingProfile, active.server, inCall]);

  const handleAuth = async () => {
    const endpoint = isRegistering ? "register" : "login";
    try { const res = await fetch(`${BACKEND_URL}/${endpoint}`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(authForm) }); const data = await res.json();
  ...
  // --- ACTIONS ---
  const sendFriendRequest = async () => { const usernameToAdd = prompt("Enter username:"); if (!usernameToAdd) return; const res = await fetch(`${BACKEND_URL}/send-request`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ myId: user.id, usernameToAdd }) }); alert((await res.json()).message); };
  ...
  const saveServerSettings = async () => { let finalImg = serverEditForm.imageUrl; if (newServerFile) { const formData = new FormData(); formData.append("file", newServerFile); finalImg = (await (await fetch(`${BACKEND_URL}/upload`, { method: "POST", body: formData })).json()).fileUrl; } await fetch(`${BACKEND_URL}/servers/update`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ serverId: active.server.id, userId: user.id, name: serverEditForm.name, imageUrl: finalImg }) }); setShowServerSettings(false); setNewServerFile(null); };
  const promoteMember = async (targetId: number) => { if(!confirm("Toggle Moderator?")) return; await fetch(`${BACKEND_URL}/servers/promote`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ serverId: active.server.id, ownerId: user.id, targetUserId: targetId }) }); };
  ...
  const startDMCall = () => {
      ...
      socket.emit("start_call", { senderId: user.id, recipientId: active.friend.id, senderName: user.username, avatarUrl: user.avatar_url, roomId });
  };
  ...
  const joinVoiceRoom = useCallback((roomId: string) => {
    ...
    navigator.mediaDevices.getUserMedia({ video: false, audio: true }).then(stream => {
      ...
      socket.on("all_users", (users) => { const arr: any[] = []; users.forEach((u: any) => { const p = createPeer(u.socketId, socket.id as string, stream); peersRef.current.push({ peerID: u.socketId, peer: p, info: u.userData }); arr.push({ peerID: u.socketId, peer: p, info: u.userData }); }); setPeers(arr); });
      socket.on("user_joined", (p) => { const item = peersRef.current.find(x => x.peerID === p.callerID); if(item) { item.peer.signal(p.signal); return; } const peer = addPeer(p.signal, p.callerID, stream); peersRef.current.push({ peerID: p.callerID, peer, info: p.userData }); setPeers(u => [...u, { peerID: p.callerID, peer, info: p.userData }]); });
      socket.on("receiving_returned_signal", (p) => { const item = peersRef.current.find(x => x.peerID === p.id); if(item) item.peer.signal(p.signal); });
  ...
  const startScreenShare = async () => { try { const s = await navigator.mediaDevices.getDisplayMedia({ video: true, audio: true }); setScreenStream(s); setIsScreenSharing(true); const t = s.getVideoTracks()[0]; peersRef.current.forEach((po) => { const pc = (po.peer as any)._pc; if(pc) { const snd = pc.getSenders().find((x: any) => x.track && x.track.kind === 'video'); if(snd) snd.replaceTrack(t); else po.peer.addTrack(t, myStream); } }); t.onended = () => stopScreenShare(); } catch(e) { console.error(e); } };
  ...
  // ✅ NEW: End Call Function (Shared logic)
...
const MediaPlayer = ({ peer }: any) => { 
    ...
    return <video ref={videoRef} autoPlay playsInline className="w-full h-full object-cover bg-black" />; 
};